...
/// <reference name="System.Web.UI.Interface.js" assembly="System.Web.UI.Interface" />
...
System.Web.UI.Interface.NewWindow = function(valUrl,valId,valWidth,valHeight,strValH,strValV,useAvailable,customOptions, requestedPopUp){
	...
	if (H.substr(H.length-1,1) == "%"){
		H = H.replace("%","");
		H = parseInt(H) * screenHeight / 100;
		H = parseInt(H);
	}
	// Calculate Horizontal position.
	var intLeft = new Number;
	var strLeft = new String(strValH);
	switch (strLeft.toLowerCase()){
		case "left":	{ intLeft = 0; break; };
		case "center":	{ intLeft = parseInt((screenWidth-W)/2); break; };
		case "right":	{ intLeft = parseInt(screenWidth-W); break; };
	...
	switch (strTop.toLowerCase()){
		...
		case "middle":	{ intTop = parseInt((screenHeight-H)/2); break; };
		case "bottom":	{ intTop = parseInt(screenHeight-H); break; };
...
System.Web.UI.Interface.Icon = function(valId,valTarget,valFile,valClassName,valWidth,valHeight){
	...
	//---------------------------------------------------------
	var me = this;
	var _imageWidth = new String;
	var _imageHeight = new String;
	//---------------------------------------------------------
	// METHOD: CreateInterface
	//---------------------------------------------------------
	this.CreateInterface = function(){
