...
/// <reference name="System.Web.UI.ShortKeys.js" assembly="System.Web.UI.ShortKeys" />
...
System.Web.UI.ShortKeys.KeysManager = function(id, control, target){
	...
	// Public properties.
	//---------------------------------------------------------
 	this.PreviousKey = new Number;
	this.PreventedKeys = {};
 	this.LastDownKey = new Number;
 	//---------------------------------------------------------
	// Private properties.
	//---------------------------------------------------------
 	var me = this;
	...
	this.OnKeyEvent = function(sender,e){
		var strKeyDown = new String;
		var strKeyDownLR = new String;
		var objActiveElement = new Object;
		...
		var enumDown = new Array();
		var enumDownFull = new Array();
		// Check if some keys are down. "R" and "L" prefix works only for IE.
		var singleDown = new Array();
		if (e.Event.shiftKey){
		...
		if (e.Name == "OnKeyPress"){
			...
			for (var i = 0; i < singleDown.length; i++){
				// Shift number by 8 bits to left.
				if (i > 0) layoutCode = layoutCode << 8;
				layoutCode = layoutCode | singleDown[i];
			}
			// If upper case, check if shift is not pressed. if lower case, check if shift is pressed
			capsLockState = (keyCode > 64 && keyCode < 91 && !e.Event.shiftKey) || ( keyCode > 96 && keyCode < 123 && e.Event.shiftKey );
			// Change KeyName.
